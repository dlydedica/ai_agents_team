...
Использование:
    ...
    python team.py members [отдел]
    python team.py hr balance|suggest|hire|rebalance
    ...
    python team.py delegate <task_id> <department>/<member>
    python team.py self-diagnose
    python team.py self-diagnose --fix
    python team.py validate-permissions
...
def self_diagnose(fix_mode: bool = False):
    ...
    """
    from checks.registry import run_all_checks, discover_checks, learn_from_failure
...
def delegate(task_id: str, target: str):
    """
    Делегировать задачу отделу или конкретному сотруднику.
    ...
    Примеры:
        python team.py delegate task-abc development
        python team.py delegate task-abc development/flutter_specialist
    """
    ...
    member_file = Path(__file__).parent / ".github" / "agents" / "members" / f"{member}.agent.md" if member else None

    # Определяем, какой agent файл использовать
    if member_file and member_file.exists():
        primary_agent = member_file
        agent_label = f"{department.capitalize()}/{member}"
    else:
        primary_agent = agent_file
        agent_label = department.capitalize()
    ...
    handoff_input = []
    handoff_output = []
    if primary_agent.exists():
        in_section = False
        out_section = False
        for line in primary_agent.read_text(encoding="utf-8").split("\n"):
    ...
    print(f"\n{'='*60}")
    print(f"{emoji} Делегирование: {agent_label}")
    print(f"{'='*60}")
    print(f"  Задача: {task.get('title', '?')}")
    print(f"  {direction}")
    print()
    if member:
        print(f"  👤 Сотрудник: {member}")
        print(f"  📄 Профиль: .github/agents/members/{member}.agent.md")
        print()
    ...
    print()
    if member:
        print(f"  💡 Вызови subagent: members/{member}.agent.md")
    else:
...
def orchestrate(description_or_file: str):
    ...
    from memory.memory_store import suggest_similar, search
...
MEMBERS_DIR = Path(__file__).parent / ".github" / "agents" / "members"


def _list_members(department: str = ""):
    """Показать сотрудников отдела."""
    members_file = MEMBERS_DIR / ".." / "departments" / f"{department}.agent.md"
    members_path = Path(__file__).parent / ".github" / "agents" / "members"

    if department:
        # Показываем сотрудников конкретного отдела
        dept_agent = members_file.resolve()
        print(f"\n👥 Сотрудники отдела {department.capitalize()}:\n")
        for f in sorted(members_path.glob("*.agent.md")):
            name = f.stem
            desc = ""
            try:
        ...
        # Показываем все отделы с сотрудниками
        ...
        for f in sorted(members_path.glob("*.agent.md")):
            ...
            for s in skills[:3]:
                print(f"      {s}")
        print()
    print(f"  💡 python team.py delegate <task_id> <department>/<member>")
...
def main():
    ...
    if command == "list-departments":
        ...
        if subcmd == "balance":
        ...
        elif subcmd == "competency":
            if len(sys.argv) < 4:
                print("❌ Подкоманды: update <dept> <skill> <grade>")
                sys.exit(1)
            hr_sub = sys.argv[3]
            if hr_sub == "update":
                if len(sys.argv) < 7:
                    print("❌ Использование: python team.py hr competency update <department> <skill> <grade>")
                else:
                    from departments.hr.hr_engine import update_competency
                    dept = sys.argv[4]
                    skill = sys.argv[5]
                    grade = sys.argv[6]
                    update_competency(dept, skill, grade)
            else:
                print(f"❌ Неизвестная подкоманда: {hr_sub}")
        else:
            ...
            print("     competency— управление компетенциями (update)")
    elif command == "members":
        dept_name = sys.argv[2] if len(sys.argv) > 2 else ""
        _list_members(dept_name)
    elif command == "delegate":
        if len(sys.argv) < 4:
            print("❌ Укажите task_id и отдел/сотрудника.")
            print("   Пример: python team.py delegate task-a1b2c3 development")
            print("   Пример: python team.py delegate task-a1b2c3 development/flutter_specialist")
            sys.exit(1)
        delegate(sys.argv[2], sys.argv[3])
    elif command == "dashboard":
        if len(sys.argv) >= 3 and sys.argv[2] == "seed":
            from pathlib import Path
            import sys as _sys
            _seed_path = Path(__file__).resolve().parent / "dashboard" / "seed_data.py"
            exec(compile(_seed_path.read_text(encoding="utf-8"), _seed_path, "exec"))
        else:
            port = 8000
            if len(sys.argv) >= 4 and sys.argv[2] == "--port":
                port = int(sys.argv[3])
            ...
            _sp.run([_sys.executable, "-m", "uvicorn", "dashboard.app:app",
                     "--host", "0.0.0.0", "--port", str(port)])
    elif command == "integrate":
        if len(sys.argv) < 3:
            print("❌ Укажите путь к целевому проекту.")
            print("   Пример: python team.py integrate /path/to/project")
            print("   Пример: python team.py integrate update /path/to/project")
            sys.exit(1)
        if sys.argv[2] == "update":
            if len(sys.argv) < 4:
                print("❌ Укажите путь к проекту: python team.py integrate update /path/to/project")
                sys.exit(1)
            from integration.update import update as _update
            _update(sys.argv[3])
        else:
            from integration.integrate import integrate as _integrate
            _integrate(sys.argv[2])
    elif command == "validate":
        ok = validate_all_python(verbose=True)
        sys.exit(0 if ok else 1)
    elif command == "validate-permissions":
        """Проверить permissions всех agent-файлов по ADR-003."""
        from checks.builtin.permissions_validator import check as validate_permissions
        ok, msg = validate_permissions()
        print(f"\n{'='*60}")
        print(f"  🛡️ Валидация permissions (ADR-003)")
        print(f"{'='*60}")
        print(f"  {msg}")
        print()
        sys.exit(0 if ok else 1)
        ...
        if len(sys.argv) < 3:
            print("❌ Подкоманды: profile list, profile assign, install-pip")
            sys.exit(1)
        subcmd = sys.argv[2]
        if subcmd == "profile":
            if len(sys.argv) >= 4 and sys.argv[3] == "list":
                from skills.manager import list_profiles
                profiles = list_profiles()
                print(f"\n📋 Профили агентов ({len(profiles)}):\n")
                for p in profiles:
                    print(f"  📦 {p.get('name', '?')} — {p.get('description', '')[:80]}")
                    skills = p.get("skills", [])
                    if skills:
                        print(f"     скилы: {', '.join(s[:20] for s in skills[:5])}")
                    print()
            elif len(sys.argv) >= 5 and sys.argv[3] == "assign":
                from skills.manager import assign_skills_to_agent
                agent = sys.argv[4]
                skills = sys.argv[5:]
                result = assign_skills_to_agent(agent, skills)
                if "error" in result:
                    print(f"❌ {result['error']}")
                else:
                    print(f"✅ Скилы назначены: {agent} ← {', '.join(skills)}")
            else:
                print("   Использование:")
                print("     python team.py skill profile list")
                print("     python team.py skill profile assign <agent> <skill...>")
        elif subcmd == "install-pip":
            if len(sys.argv) < 4:
                print("❌ Укажите pip-пакет: python team.py skill install-pip <package>")
            else:
                from skills.loader import install_from_pip
                result = install_from_pip(sys.argv[3])
                if "error" in result:
                    print(f"❌ {result['error']}")
                else:
                    print(f"✅ Установлен: {sys.argv[3]}")
        elif subcmd == "install-symlink":
            if len(sys.argv) < 4:
                print("❌ Укажите путь: python team.py skill install-symlink <path> [name]")
            else:
                from skills.loader import install_from_symlink
                src = sys.argv[3]
                name = sys.argv[4] if len(sys.argv) > 4 else ""
                result = install_from_symlink(src, name)
                if "error" in result:
                    print(f"❌ {result['error']}")
                else:
                    print(f"✅ Симлинк создан: {name or src}")
        else:
            print(f"❌ Неизвестная подкоманда skill: {subcmd}")
