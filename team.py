...
def self_diagnose(fix_mode: bool = False):
    """
    Полная самодиагностика команды: синтаксис, импорты, файлы, тесты.
    При fix_mode=True пытается исправить найденные проблемы.
    """
    EP = "\033[92m✅\033[0m"
    EF = "\033[91m❌\033[0m"
    EW = "\033[93m⚠️\033[0m"
    ...
    print(f"\n{'='*60}")
    print(f"  🏥 AI DevCorp — Полная самодиагностика")
    print(f"{'='*60}\n")

    results = {"passed": 0, "failed": 0, "warnings": 0}
    issues = []

    def check(name, ok, desc, fail_desc=""):
        if ok:
            results["passed"] += 1
            print(f"  {EP} {name}: {desc}")
        else:
            results["failed"] += 1
            print(f"  {EF} {name}: {fail_desc or desc}")
            issues.append({"check": name, "detail": fail_desc or desc})

    # 1. Синтаксис Python
    checked, errors = validate_all_python(verbose=False)
    check("Синтаксис Python", not errors,
          f"{checked} файлов валидны", f"{len(errors)} ошибок")

    # 2. Критические файлы
    critical = ["team.py", "mcp-server/server.py", "mcp-server/task_store.py",
                 "orchestration/ceo.md", ".github/agents/orchestrator.agent.md"]
    missing = [f for f in critical if not (repo / f).exists()]
    check("Критические файлы", not missing,
          "все на месте", f"отсутствуют: {missing}")

    # 3. Импорты
    import_errors = []
    try:
        import mcp_server  # noqa: F401
        exec("from task_store import get_all_tasks", {"__builtins__": __builtins__})
    except Exception as e:
        import_errors.append(f"task_store: {e}")
    try:
        exec("from memory.memory_store import search", {"__builtins__": __builtins__})
    except Exception as e:
        import_errors.append(f"memory_store: {e}")
    try:
        exec("from dashboard.app import app", {"__builtins__": __builtins__})
    except Exception as e:
        import_errors.append(f"dashboard: {e}")
    check("Импорты модулей", not import_errors,
          "все модули импортируются", "; ".join(import_errors))

    # 4. Департаменты + агенты
    depts = [d for d in DEPARTMENTS_DIR.iterdir() if d.is_dir()]
    agents_dir = repo / ".github" / "agents" / "departments"
    agents = list(agents_dir.glob("*.agent.md")) if agents_dir.exists() else []
    check("Агенты покрывают отделы", len(depts) == len(agents),
          f"{len(agents)} агентов = {len(depts)} отделов",
          f"{len(agents)} агентов != {len(depts)} отделов")
    ...
    try:
        ...
        t = create_task("_healthcheck_", ["development"])
        t2 = get_task(t["id"])
        mcp_ok = t2 is not None
    except Exception:
        pass
    check("MCP-сервер", mcp_ok, "работает", "не отвечает")
    ...
    try:
        import subprocess
        r = subprocess.run(
            [sys.executable, str(repo / "mcp-server" / "test_task_store.py")],
            capture_output=True, text=True, timeout=15
        )
        test_ok = "Пройдено: 13" in r.stdout and "Провалено: 0" in r.stdout
    except Exception:
        pass
    check("Тесты (core)", test_ok, "13/13 пройдено", "не пройдены")

    # Итог
    print(f"\n{'='*60}")
    total = results["passed"] + results["failed"]
    if results["failed"] == 0:
        print(f"  ✅ **СТАТУС: ЗДОРОВ**")
    else:
        print(f"  ❌ **СТАТУС: ТРЕБУЕТ ВНИМАНИЯ**")
    print(f"  ✅ Пройдено: {results['passed']}")
    print(f"  ❌ Ошибок: {results['failed']}")
    print(f"{'='*60}\n")

    # Auto-fix
    if fix_mode and results["failed"] > 0:
        print(f"🔧 Запуск автоисправления...\n")
        fixes = _auto_fix()
        if fixes:
            for f in fixes:
                print(f)
            print(f"\n✅ Применено {len(fixes)} исправлений")
            print(f"   Запустите self-diagnose повторно для проверки\n")
        else:
            print(f"  Нет автоматических исправлений\n")
